...
void callback(const pcl::PointCloud<pcl::PointXYZRGB>::ConstPtr& msg)
{
  if(empieza==true){
    ...
    vGrid.setInputCloud (cloud_1);
    vGrid.setLeafSize (0.05f, 0.05f, 0.05f);
    vGrid.filter (*cloud_1);
    cout << "Puntos tras VG_1: " << cloud_1->size() << endl;

    std::vector<int> indices;
    cloud_1->is_dense = false;
    removeNaNFromPointCloud<PointXYZRGB>(*cloud_1, *cloud_1, indices);
    cout << "Quitamos los NAN y quedan: " << cloud_1->size() << endl;
